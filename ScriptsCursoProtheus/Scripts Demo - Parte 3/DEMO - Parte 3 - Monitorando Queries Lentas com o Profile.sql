...
-- Create Database Curso_Protheus

----------- Steps to execute this script and create a Profile to monitor 3 seconds query

-- 1) Fazer um Replace(CTRL + H) no caminho "C:\Temp" e alterar para um Caminho Real no servidor. Você precisa dar acesso ao usuário do SQL nessa pasta para criar e excluir arquivo, caso contrário o job de trace vai falhar.

-- 1) You need to do a replace the path "C:\Temp" on this script to a path that exists on the server. You need to give access to the SQL User on this path to create and drop the trace file. Without this, the job will fail.
...
USE Curso_Protheus -- Database Name
...
BEGIN
	...
	*******************************************************************************************************************************/
	exec @rc = sp_trace_create @TraceID output, 0, N'C:\Temp\Duration', @maxfilesize, NULL 
...
BEGIN
	...
	EndTime, Reads,Writes, CPU, ServerName, DataBaseName, RowCounts/*, SessionLoginName*/
	FROM :: fn_trace_gettable('C:\Temp\Duration.trc', default)
...
BEGIN TRANSACTION
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep 
			...
			@os_run_priority = 0,
			@subsystem=N'CmdExec',
			@command = N'Del "C:\Temp\Duration.trc" /Q',
			@flags = 0
