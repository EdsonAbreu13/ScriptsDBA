...
select    SUBSTRING(TextData, 1, 60) as obj, sum(CPU) as CPU_Total, sum(Reads) as Reads_Total, count(*) as QTD_Requests from    Traces..Queries_Profile where    StartTime > '20200701' --and    ((textdata like 'exec sp[0-9]%') or (textdata like 'exec sr[0-9]%')) group by SUBSTRING(TextData, 1, 60) order by 4 desc  
